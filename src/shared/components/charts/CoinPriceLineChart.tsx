...
interface CoinPriceLineChartProps {
  coinId: number;
  selectedDate: string;
  tradingPrice: number;
  isBuy: boolean;
...
export default function CoinPriceLineChart({
  coinId,
  selectedDate,
  tradingPrice,
  isBuy,
  avgBuyPrice,
  containerClassName = '',
}: CoinPriceLineChartProps) {
  const chartContainerRef = useRef<HTMLDivElement>(null);
  const chartRef = useRef<IChartApi | null>(null);
  const seriesRef = useRef<ISeriesApi<'Line'> | null>(null);
  const priceLineRef = useRef<any>(null);
  const avgBuyPriceLineRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (chart && series) {
      ...
      if (series && !isBuy && avgBuyPrice && avgBuyPrice > 0) {
      ...
      } else if (avgBuyPriceLineRef.current && series) {
