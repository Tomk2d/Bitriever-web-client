import type { CommunityContentBlock, ParsedCommunityContent } from '../types';
...
export function parseContentToText(content: string | null | undefined): string {
  ...
  try {
    const parsed: ParsedCommunityContent = JSON.parse(content);
    if (!parsed.blocks || !Array.isArray(parsed.blocks)) {
      return content;
    }
    
    // 각 블록을 순서대로 연결 (사용자가 입력한 그대로 복원)
    return parsed.blocks.map((block: CommunityContentBlock) => {
      if (block.type === 'text') {
 ...
 */
export function textToContentBlocks(text: string, communityId: number): ParsedCommunityContent {
  if (!text) {
    return { blocks: [] };
  }
  ...
  while ((match = imageMarkerRegex.exec(text)) !== null) {
    ...
    const imagePath = `@communityImage/${communityId}/${filename}`;
